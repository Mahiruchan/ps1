...
function Start-Edge {
    Write-Output "启动 Edge Kiosk 模式: `$url"
    Start-Process `$edge "--kiosk `$url --edge-kiosk-type=fullscreen"
}

Write-Output "=== Kiosk 监控程序启动 ==="
Write-Output "目标URL: `$url"
Write-Output "内存阈值: `$memoryThresholdMB MB"
Write-Output "停止方法: 按 Ctrl+C 或创建文件 `$stopFlagFile"
Write-Output "程序标识: Kiosk Monitor - `$url"
Write-Output ""
...
function Uninstall-ExistingKiosk {
    ...
    if ($kioskProcesses) {
        Write-Host "发现运行中的Kiosk监控进程，正在停止..." -ForegroundColor Yellow
        $kioskProcesses | Stop-Process -Force
        Write-Host "已停止现有Kiosk监控进程" -ForegroundColor Green
    }
...
function Install-KioskScript {
    ...
    Write-Host "已创建Kiosk监控脚本: $scriptPath" -ForegroundColor Green
...
"@
    ...
    switch ($choice) {
        '1' {
            try {
                # 创建Windows任务计划
                $taskName = "KioskMonitor"
                $taskDescription = "Kiosk模式监控程序 - 开机自启动"
    ...
    # 询问是否立即启动
    $start = Read-Host "是否立即启动Kiosk监控? (y/N)"
    if ($start -eq 'y' -or $start -eq 'Y') {
        Write-Host "正在启动Kiosk监控..." -ForegroundColor Yellow
        Start-Process PowerShell -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$scriptPath`""
        Write-Host "Kiosk监控已启动! (窗口标题: Kiosk Monitor - URL)" -ForegroundColor Green
    }
    
    Write-Host "`n安装完成" -ForegroundColor Green
    Write-Host ""
    Write-Host "文件位置:" -ForegroundColor Cyan
    Write-Host "  监控脚本: $scriptPath" -ForegroundColor White
    Write-Host "  启动脚本: $startupPath" -ForegroundColor White
    Write-Host ""
    Write-Host "使用方法:" -ForegroundColor Cyan
    Write-Host "  启动监控: PowerShell -ExecutionPolicy Bypass -File `"$startupPath`"" -ForegroundColor White
    Write-Host "  停止监控: 在监控窗口按 Ctrl+C 或创建文件 C:\KioskMonitor\stop.flag" -ForegroundColor White
    ...
    Write-Host "  监控程序运行时窗口标题显示 'Kiosk Monitor - URL'" -ForegroundColor Gray
...
try {
    ...
    Write-Host "  [I] 安装 Kiosk 监控服务" -ForegroundColor White
    Write-Host "  [R] 移除 Kiosk 监控服务" -ForegroundColor White
    Write-Host ""
    ...
    if ($operation -eq 'R') {
        # 执行卸载操作
        Write-Host ""
        Write-Host "=== 开始卸载 Kiosk 监控服务 ===" -ForegroundColor Red
        Uninstall-ExistingKiosk
        Write-Host ""
        Write-Host "卸载完成" -ForegroundColor Green
        Write-Host "所有 Kiosk 监控服务已从系统中移除" -ForegroundColor Cyan
        return
    }
    
    # 执行安装操作
    Write-Host ""
    Write-Host "=== 开始安装 Kiosk 监控服务 ===" -ForegroundColor Green
